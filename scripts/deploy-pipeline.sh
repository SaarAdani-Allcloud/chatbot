...
TFVARS
  ...
  info "Generating backend.hcl with S3 state configuration..."
  cat > "$TF_DIR/backend.hcl" <<BACKEND
...
use_lockfile = true
BACKEND
  ok "Generated $TF_DIR/backend.hcl"
...
if [[ -f "terraform/pipeline/terraform.tfvars" ]]; then
  git add -f terraform/pipeline/terraform.tfvars
  GITIGNORED_FILES+=("terraform/pipeline/terraform.tfvars")
fi

if [[ -f "terraform/pipeline/backend.hcl" ]]; then
  git add -f terraform/pipeline/backend.hcl
  GITIGNORED_FILES+=("terraform/pipeline/backend.hcl")
fi
